function [ problem ] = RSMDP_GenerateRiver( n, m, pTerra, pAgua, custoTerra, custoAgua )
%RSMDP-GENERATE Summary of this function goes here
%   Detailed explanation goes here
%   n = number of lines
%   m = number of rows
%   pTerra = Probabilidade de sucesso em terra
%   pAgua = Probabilidade de sucesso em agua
%   custoTerra = custo em Terra
%   custoAgua = custo em Agua
...
if nargin < 3; pTerra = 0.99; end;
if nargin < 4; pAgua = 0.80; end;
if nargin < 5; custoTerra = 1; end;
if nargin < 6; custoAgua = 1; end;
...
problem.reward2D(:, :) = +custoAgua; %Cada movimento (ação), tem custo -1; (custo está positivo devido algoritmo 4)
...
problem.reward2D([stateLandLeft,stateLandRight,stateBridge], :) = +custoTerra; %Cada movimento em terra tem custo maior
...
problem.transition([stateLandLeft,stateLandRight,stateBridge],:,[1,2,3,4]) = problem.transition([stateLandLeft,stateLandRight,stateBridge],:,[1,2,3,4]) + problem.maskActions([stateLandLeft,stateLandRight,stateBridge],:,[1,2,3,4]) .* pTerra;
problem.transition([stateLandLeft,stateLandRight,stateBridge],:,1) = problem.transition([stateLandLeft,stateLandRight,stateBridge],:,1) + problem.maskActions([stateLandLeft,stateLandRight,stateBridge],:,5) .* (1 - pTerra);
problem.transition([stateLandLeft,stateLandRight,stateBridge],:,2) = problem.transition([stateLandLeft,stateLandRight,stateBridge],:,2) + problem.maskActions([stateLandLeft,stateLandRight,stateBridge],:,5) .* (1 - pTerra);
problem.transition([stateLandLeft,stateLandRight,stateBridge],:,3) = problem.transition([stateLandLeft,stateLandRight,stateBridge],:,3) + problem.maskActions([stateLandLeft,stateLandRight,stateBridge],:,5) .* (1 - pTerra);
problem.transition([stateLandLeft,stateLandRight,stateBridge],:,4) = problem.transition([stateLandLeft,stateLandRight,stateBridge],:,4) + problem.maskActions([stateLandLeft,stateLandRight,stateBridge],:,5) .* (1 - pTerra);

% Mover para oeste (2), na margem esquerda é igual ficar parado (5)
problem.transition(stateLandLeft,:,2)  = problem.transition(stateLandLeft,:,2)  + problem.maskActions(stateLandLeft,:,5) .* pTerra;
% Mover para leste (3), na margem direita é igual ficar parado (5)
problem.transition(stateLandRight,:,3) = problem.transition(stateLandRight,:,3) + problem.maskActions(stateLandRight,:,5) .* pTerra;
% Mover para norte (1), na ponte é igual ficar parado (5)
problem.transition(stateBridge,:,1) = problem.transition(stateBridge,:,1) + problem.maskActions(stateBridge,:,5) .* pTerra;
% Mover para norte (1), nas duas margens de entrada da ponte é igual ficar parado (5)
problem.transition([1,m],:,1) = problem.transition([1,m],:,1) + problem.maskActions([1,m],:,5) .* pTerra;

% Mover para sul (4), no start é igual ficar parado (5)
problem.transition(stateStart,:,4) = problem.transition(stateStart,:,4) + problem.maskActions(stateStart,:,5) .* pTerra;
...
problem.transition(stateWater,:,2) = problem.transition(stateWater,:,2)  + problem.maskActions(stateWater,:,2) .* pAgua;
problem.transition(stateWater,:,2) = problem.transition(stateWater,:,2)  + problem.maskActions(stateWater,:,4) .* (1 - pAgua);

% Mover para leste (3), no rio é 80% possível e 20% de
problem.transition(stateWater,:,3) = problem.transition(stateWater,:,3)  + problem.maskActions(stateWater,:,3) .* pAgua;
problem.transition(stateWater,:,3) = problem.transition(stateWater,:,3)  + problem.maskActions(stateWater,:,4) .* (1 - pAgua);
