function [ saidaVl, saidaI ] = MaxTudo2( matrixT )
%MAXTUDO COmpara tudo utilizando a classe CNumero
...
for dim1 = 1:sizeDims(1) %dim1 = 2
    ...
    for dim2 = 1:sizeDims(2)
        CNum{dim2} = CNumero(0);
        for dim3 = 1:sizeDims(3)
            CNum{dim2} = CNum{dim2}.somaObj( CNumero( C(dim2, dim3)) );
        end
    end
    ...
    for dim2 = 1:sizeDims(2)
        if dim2 == 1
            ...
            if CNum{maiorAcao}.maior( CNum{dim2} ) ~= 1
                maiorAcao = dim2;
            end
        end
